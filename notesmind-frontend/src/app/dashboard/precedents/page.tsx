...
export default function PrecedentsDirectoryPage() {
  ...
  useEffect(() => {
    const fetchResults = async (searchQuery: string) => {
      ...
      try {
        const res = await api.get(`/knowledge/search?query=${encodeURIComponent(searchQuery)}&type=precedent`, {
    ...
    const timer = setTimeout(() => {
      const q = query.trim() || "notesheet precedent cases";
      fetchResults(q);
    }, 500);
  ...
  return (
    <div className="flex flex-col h-full bg-[var(--color-khadi)]">
      <div className="h-28 bg-white border-b border-[#e5e1d8] flex flex-col justify-center px-10 shrink-0 shadow-sm z-10 relative">
        <h1 className="font-document text-2xl font-bold text-[#2c3e50] mb-2">Past Precedents</h1>
        <div className="relative w-full max-w-2xl">
          ...
          <input
            ...
            placeholder="Search past notesheets, previous approvals..."
          ...
          {loading && initialLoad ? (
            ...
            results.map((r, i) => (
              <div key={i} className="citation-card relative overflow-hidden bg-white p-5 rounded-xl shadow-sm border border-[#e5e1d8]">
                ...
                <span className="inline-block px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-widest bg-purple-50 text-purple-700 border border-purple-200 mb-2">Precedent: {r.id}</span>
            ...
            <div className="text-center py-20 opacity-50">
              <Layers size={48} className="mx-auto mb-4 text-[var(--color-umber-light)]" strokeWidth={1.5}/>
              <p className="font-ui text-sm text-[var(--color-umber-light)]">No precedents found for "{query}".</p>
            </div>
          )}
        </div>
      </div>
